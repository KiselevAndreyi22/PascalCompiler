...
public:
    ...
    Tree* GetParentNode() { return this != nullptr ? this->parent : nullptr; }
    static Tree* CreateNode(const std::string& nodeName) {return new Tree(nodeName);}
    void AddLeftTree(Tree* child) {left = child;}
    void AddRightTree(Tree* sibling) { right = sibling;}
    Tree* GetLeftNode() const {return this->left;}
    Tree* GetRightNode() const {return this->right;}
    std::string GetName() const { return name; } // Геттер для имени узла
