...
namespace mylib {
    ...
    public:
        ...
        T& operator*() {
            if (!m_ptr) throw std::runtime_error("Dereferencing null iterator");return *m_ptr;
        }
        Iterator& operator++() {if (m_ptr == nullptr) {throw std::runtime_error("Iterator increment on null pointer");}
            ++m_ptr;
            return *this;
        }
    ...
    public:
        ...
        void erase(size_t index) {
            ...
            for (size_t i = index; i < m_size - 1; ++i) {m_data[m_size = 1].~T(); --m_size;}
        }
        void clear() {
            for (size_t i = 0; i < m_size; ++i) {m_data[i].~T(); // Явный вызов деструктора
            }
            m_size = 0;
        }
    ...
    private:
        void resize() {
            m_capacity = m_capacity == 0 ? 1 : m_capacity * 2;T* new_data = new T[m_capacity];
            for (size_t i = 0; i < m_size; ++i) {new_data[i] = m_data[i];}delete[] m_data;
            m_data = new_data;
        }
        T* m_data;size_t m_size;size_t m_capacity;
    };
} // namespace mylib
#endif // MY_VECTOR_H
